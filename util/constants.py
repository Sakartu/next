...
SHOW_REGEXES = [
    r'(?P<show>{show}).*S0*(?P<season>{season})E0*(?P<ep>{ep}).*',  #accounts for most S12E12 type of shows
    r'(?P<show>{show}).*0*(?P<season>{season})x0*(?P<ep>{ep}).*'    #accounts for most 12x12 type of shows
    ] 

...
def update_eps(db):
    ...
    try:
        for show in all_shows:
            ...
            db.store_tvr_eps(all_eps)
    except:#probably no internet connection
        print "Could not connect to TVRage, aborting update!"
        return

    process_maybe_finished(db, all_shows)
    print "done."

def process_maybe_finished(db, all_shows):
    '''
    ...
    for show in all_shows:
        if show.maybe_finished:
            next_ep = db.find_next_ep(show)
            if next_ep:
                db.change_show(show.sid, next_ep.season, next_ep.epnum)
                db.mark_not_maybe_finished(show.sid)

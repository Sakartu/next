...
from constants import TVRage as tvrpath


def fuzzy_search(show_name):
    '''
    This method searches the TVRage database for a given showname. The search
    is fuzzy, so looking for "doctor" will return Doctor Who (2005) as well.
    The result is a list of (name, sid) tuples.
    '''
    ...
    results = map(lambda (x, y, z): (x.text, y.text, z.text), zip(names, ids,
        statuses))
    return results
...
def get_all_eps(sid):
    '''
    This method parses episode list from the TVRage database for a given sid
    and returns an Episode object for each episode in the list
    '''
    ...
    for season in seasons:
        ...
        for ep in eps:
            ...
            results.append(Episode(sid, showname, seasonnum, epnum, title,
                airdate))
    return results
